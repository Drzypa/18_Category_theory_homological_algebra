...
\usepackage{pmmeta}
\pmcanonicalname{DerivedCategory}
\pmcreated{2013-03-22 13:27:05}
\pmmodified{2013-03-22 13:27:05}
\pmowner{mathcam}{2727}
\pmmodifier{mathcam}{2727}
\pmtitle{derived category}
\pmrecord{8}{34016}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $\ES{A}$ be an abelian category, and let $\mc{K}(\ES{A})$ be the category of
chain complexes in $\ES{A}$, with the morphisms being chain homotopy classes of maps.  
Call a morphism of chain complexes a \emph{quasi-isomorphism} if it induces an 
isomorphism on homology groups of the complexes.  For example, any chain homotopy 
is a quasi-isomorphism, but not conversely.  Now let the derived category $\mc{D}(\ES{A})$ 
be the category obtained from $\mc{K}(\ES{A})$ by adding a formal inverse to 
every quasi-isomorphism (technically this called a localization of the 
category).

Derived categories seem somewhat obscure, but in fact, many mathematicians 
believe they are the appropriate place to do homological algebra.  One of their
great advantages is that the important functors of homological algebra which are 
left or right exact ($\mathrm{Hom}$,$N\otimes_k-$, where $N$ is a fixed $k$-module, 
the global sections functor $\Gamma$, etc.) become exact on the \PMlinkescapetext{level} of
derived functors (with an appropriately modified definition of exact).

See \emph{Methods of Homological Algebra}, by Gelfand and Manin for more details.
%%%%%
%%%%%
\end{document}
